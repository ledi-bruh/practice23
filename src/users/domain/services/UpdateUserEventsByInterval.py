...
class UpdateUserEventsByInterval:
    ...
    async def __check_interval_for_events(
        self,
    ...
    async def __call__(
        ...
        await self.__check_interval_for_events(interval, events_to_create)
        await self.trim_user_events_by_interval(user, interval)
